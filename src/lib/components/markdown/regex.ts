 ...
 */
export const DiscordEmojiRegex = /^<(?<animated>a)?:(?<name>\w{2,32}):(?<id>\d{17,21})>/g;
...
export const InclusiveSpoilerRegex = /\|\|([\s\S]+?)\|\|/;
...
export const UnderlineRegex = /^__([\s\S]+?)__(?!_)/;
...
// These regular expressions require RegExp.prototype.unicodeSets
// Which means target must be set to ESnext
// https://caniuse.com/?search=RegExp.prototype.unicodeSets

/**
 * Regex that can capture an Emoji
 */
export const EmojiRegex = /^(\p{RGI_Emoji})/v;
/**
 * Matches if an emoji exists in the string
 */
export const EmojiExistsRegex = /(\p{RGI_Emoji})/v;